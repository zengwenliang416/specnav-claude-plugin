...
HELM_PROTOTYPE_ROOT="$(helm_plugin_root helm-prototype)"
HELM_MARKETPLACE_ROOT="$(dirname "$(dirname "$HELM_PROTOTYPE_ROOT")")"
node "$HELM_CORE_ROOT/scripts/plugin-suite.js" require --marketplace-root "$HELM_MARKETPLACE_ROOT" --plugin helm-core --plugin helm-requirements --plugin helm-prototype --json
```
...
```bash
node "$HELM_PROTOTYPE_ROOT/scripts/prototype-contract.js" --json
```

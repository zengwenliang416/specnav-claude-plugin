...
Resolve `SPECNAV_VERIFICATION_ROOT` with the owning Claude Code plugin resolver. If
...
2. Validate the current verification state:
   ...
   ```bash
   node "$SPECNAV_VERIFICATION_ROOT/scripts/claude-verification-adapter.js" validate \
     --project "$PWD" \
     --json
   ```
...
7. Execute the approved snapshot through the V2 adapter:
   ...
   ```bash
   node "$SPECNAV_VERIFICATION_ROOT/scripts/claude-verification-adapter.js" execute \
     --project "$PWD" \
     --change "<change-id>" \
     --reviewer-id "<authenticated-human-id>" \
     --json
...
- Run `node "$SPECNAV_VERIFICATION_ROOT/scripts/claude-verification-adapter.js" validate --project "$PWD" --change "<change-id>" --reviewer-id "<authenticated-human-id>" --json`.

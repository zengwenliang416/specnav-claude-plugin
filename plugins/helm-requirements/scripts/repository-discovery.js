...
const SCHEMA = 'helm.repositoryDiscovery.v1';
const DISCOVERY_PATH = path.join('openspec', '.helm', 'context', 'repository-discovery.json');
...
function addNegotiationSignals(context, packageScan, configs, directories, testRefs) {
  ...
  if (packageScan && packageScan.evidenceId && Object.keys(packageScan.scripts).length === 0) {
    addOpenItem(
      ...
      'Which build, test, and validation commands should Helm use for this repository?',
      { spec: 'system-architecture', section: 'Operational Constraints' },
...
function markdown(discovery, writeFile = null) {
  const lines = [];
  lines.push('# Helm Repository Discovery');
  lines.push('');

...
function installedDiscoveryAllowed(marketplaceRoot, options = {}) {
  return options.installedDiscovery === true
    || process.env.SPECNAV_ALLOW_INSTALLED_PLUGIN_DISCOVERY === '1'
    || isNonEmpty(process.env.SPECNAV_PLUGIN_LIST_JSON)
...
function readClaudePluginList(marketplaceRoot) {
  if (isNonEmpty(process.env.SPECNAV_PLUGIN_LIST_JSON)) {
    try {
      const value = JSON.parse(process.env.SPECNAV_PLUGIN_LIST_JSON);
      if (!Array.isArray(value)) {
        return { ok: false, status: 'malformed', plugins: [], error: 'SPECNAV_PLUGIN_LIST_JSON must be an array' };
      }
      return { ok: true, status: 'ok', plugins: value, source: 'SPECNAV_PLUGIN_LIST_JSON' };
    } catch (_error) {
      return { ok: false, status: 'malformed', plugins: [], error: 'SPECNAV_PLUGIN_LIST_JSON is invalid JSON' };
    }
  }
...
function installedPluginRecord(marketplaceRoot, marketplaceName, pluginName, inventoryPlugin) {
  ...
  const stage = root ? readJson(path.join(root, 'specnav-stage.json')) : { ok: false, status: 'missing' };
  const pluginMetadataValid = pluginJson.ok && isValidPluginMetadata(pluginJson.value);
  const stageManifestValid = stage.ok && isValidStageManifest(stage.value);
  const pluginNameMatches = pluginJson.ok
    && isObject(pluginJson.value)
...
function installedMarketplaceSuite(marketplaceRoot, options = {}) {
  const marketplaceName = options.marketplaceName || path.basename(path.resolve(marketplaceRoot)) || 'specnav-marketplace';

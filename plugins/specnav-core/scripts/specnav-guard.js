...
function main() {
  ...
  if (!hasOpenSpec) {
    ...
    softDeny(root, 'missing-openspec', 'missing openspec/ blocks production work. Fix: run /specnav-bootstrap to initialize or repair OpenSpec first.');
  }
  ...
  if (lightGateBlockers.length) {
    softDeny(root, lightGateBlockers[0], `light lane production edits require a valid light-change.json or light-gate.json (${lightGateBlockers.join(', ')}). Fix: run /specnav-implement and the specnav-light-change skill to create the compact entry gate.`);
  }

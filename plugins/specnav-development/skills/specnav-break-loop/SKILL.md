...
2. Read `task-ledger.jsonl`, the task `report.md`, and the review files to identify the recurring cause. Loop DETECTION is deterministic: the contract raises `loop-detected:<task>` after 3 consecutive same-cause failures in the ledger (threshold via `SPECNAV_LOOP_THRESHOLD`); this skill owns classification and escalation, not detection.
...
6. Record the classification and escalation decision as a ledger entry with status `escalated` (or `split`/`replanned`); this clears the `loop-detected` blocker. Do not re-dispatch the same failing task unchanged.

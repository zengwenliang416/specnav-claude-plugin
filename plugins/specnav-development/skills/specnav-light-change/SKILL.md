...
description: Use this skill when SpecNav routes a simple docs, copy, label, comment, or low-risk styling/config request to the light lane so the change can proceed without the full requirements/prototype/development packet.
---
...
6. Only edit files listed by the generated `scope.json`. Claude Code hooks
   block light-lane production edits when `light-gate.json` is missing or not
   ready.
...
8. Do not fabricate user testing. Before `/specnav-verify` can go green, the

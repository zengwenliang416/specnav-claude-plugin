...
8. Create each task packet with `brief.md` and `context.json`. Task packet IDs
   ...
   `001-dashboard-summary`. Write task tests first and list their paths in
   `context.json` `test_paths`; once a listed test file exists, the guard freezes
   it for the rest of implementation (changing it requires a `frozen-tests`

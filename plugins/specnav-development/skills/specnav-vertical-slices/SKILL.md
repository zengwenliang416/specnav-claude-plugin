...
7. Create each task packet with `brief.md` and `context.json`. Write task tests first and list their paths in `context.json` `test_paths`; once a listed test file exists, the guard freezes it for the rest of implementation (changing it requires a `frozen-tests` override with a reason).

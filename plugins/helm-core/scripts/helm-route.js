...
const ROUTES = {
  bootstrap: {
    target_plugin: 'helm-core',
    command: '/helm-bootstrap',
    skill: 'helm-bootstrap',
    required_plugins: ['helm-core'],
    action: 'bootstrap'
  },
  foundation: {
    target_plugin: 'helm-requirements',
    command: '/helm-requirements',
    skill: 'helm-foundation-specs',
    skills: ['helm-repository-discovery', 'helm-foundation-specs'],
    required_plugins: ['helm-core', 'helm-requirements'],
    action: 'propose'
  },
  requirements: {
    target_plugin: 'helm-requirements',
  ...
  prototype: {
    target_plugin: 'helm-prototype',
    command: '/helm-prototype',
    skill: 'helm-prototype',
    required_plugins: ['helm-core', 'helm-requirements', 'helm-prototype'],
    action: null
  },
  development: {
    target_plugin: 'helm-development',
    command: '/helm-implement',
    skill: 'helm-development-entry',
    required_plugins: ['helm-core', 'helm-development'],
    action: 'implement'
  },
  fix: {
    target_plugin: 'helm-development',
    command: '/helm-implement',
    skill: 'helm-fix',
    required_plugins: ['helm-core', 'helm-development', 'helm-verification'],
    action: 'fix'
  },
  verification: {
    target_plugin: 'helm-verification',
    command: '/helm-verify',
    skill: 'helm-verify-plan',
    required_plugins: ['helm-core', 'helm-verification'],
    action: 'verify'
  },
  release: {
    target_plugin: 'helm-operations',
    command: '/helm-release',
    skill: 'helm-release-plan',
    required_plugins: ['helm-core', 'helm-verification', 'helm-operations'],
    action: 'release',
    confirmation_required: true
  },
  archive: {
    target_plugin: 'helm-operations',
    command: '/helm-archive',
    skill: 'helm-branch-finish',
    required_plugins: ['helm-core', 'helm-verification', 'helm-operations'],
    action: 'archive',
    confirmation_required: true
  },
...
function parseArgs(args) {
  const blockers = [];
  let intent = '';
  let marketplaceRoot = process.env.HELM_MARKETPLACE_ROOT || defaultMarketplaceRoot();
  let json = false;
  const positional = [];
...
function validateFoundation(projectRoot, suiteStatus) {
  const requirementsRoot = pluginRoot(suiteStatus, 'helm-requirements');
  const coreRoot = pluginRoot(suiteStatus, 'helm-core');
  if (!requirementsRoot) return { ok: false, blockers: ['missing-plugin:helm-requirements'], specs: [] };
  if (coreRoot) process.env.HELM_CORE_ROOT = coreRoot;
  process.env.HELM_REQUIREMENTS_ROOT = requirementsRoot;

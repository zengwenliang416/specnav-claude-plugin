...
function pluginRootFrom(start) {
  ...
  while (true) {
    ...
    if (parent === current) {
      throw new Error(`helm-runtime-current-plugin-root-not-found:${path.resolve(start)}`);
    }
    current = parent;
  }
}
...
function envName(pluginName) {
  return `HELM_${pluginName.replace(/-/g, '_').toUpperCase()}_ROOT`;
}
...
function resolvePluginRoot(pluginName) {
  ...
  const envMarketplace = process.env.HELM_MARKETPLACE_ROOT
    ? path.resolve(process.env.HELM_MARKETPLACE_ROOT)
    : null;

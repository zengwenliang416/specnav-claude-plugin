...
function main() {
  const result = gate();
  if (result.ok) {
    console.log(`Helm archive gate passed for ${result.active_change}.`);
  } else {
    console.error(`Helm archive gate blocked for ${result.active_change || 'none'}: ${result.blockers.join(', ')}`);
  }

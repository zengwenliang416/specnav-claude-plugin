...
function envName(pluginName) {
  const normalized = pluginName.startsWith('helm-') ? pluginName.slice('helm-'.length) : pluginName;
  return `HELM_${normalized.replace(/-/g, '_').toUpperCase()}_ROOT`;
}
...
function installedMarketplaceRootFromCurrent() {
  ...
  if (path.basename(marketplaceRoot) !== 'helm-marketplace') return null;
...
function defaultMarketplaceRoot() {
  return path.resolve(
    process.env.HELM_MARKETPLACE_ROOT
      || installedMarketplaceRootFromCurrent()
      || path.join(os.homedir(), '.claude', 'plugins', 'cache', 'helm-marketplace')
  );
}
...
function toShell(result) {
  const lines = [];
  lines.push(`HELM_MARKETPLACE_ROOT=${shellQuote(result.marketplace_root)}`);
  for (const plugin of result.plugins) {
    if (plugin.env && plugin.root) lines.push(`${plugin.env}=${shellQuote(plugin.root)}`);
  }
  const exports = ['HELM_MARKETPLACE_ROOT', ...result.plugins.map((plugin) => plugin.env).filter(Boolean)];

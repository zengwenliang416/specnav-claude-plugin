...
3. Create or repair `scope.json` with clean relative allowed roots, denied roots, review roots, operation booleans, prototype sources, and `expires_when`.
4. Bind prototype sources to the approved prototype entry only.
5. If requested work exceeds scope, report `NEEDS_CONTEXT` or `BLOCKED` instead of expanding silently.

...
4. If `scope.json` is missing, run `node "$HELM_DEVELOPMENT_ROOT/skills/helm-scope-lock/scripts/create-scope-lock.js" --json`.
...
- Rerun `node "$HELM_DEVELOPMENT_ROOT/scripts/development-contract.js" --mode entry --json` and proceed only when scope blockers are gone.

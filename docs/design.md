...
1. Soft-gate warning dedup (once per session per reason+change; events keep
   full history) and requirements-stage awareness (docs/markdown edits under
   a requirements-only change no longer warn about missing tasks).
...
## Lane Routing (risk-tier v2)

`risk-tier.js` classifies every change into a lane recorded in
`openspec/changes/<change>/risk-tier.json`:

| Lane | Tier | Prototype | Reviews | Verification domains | Release |
| --- | --- | --- | --- | --- | --- |
| light | lite | `decision.json` may be `not_required` (reason mandatory) | spec review (quality folds in) | static + unit | standard gates |
| standard | standard | required, full artifact suite | spec + quality (forked verifier) | all six | standard gates |
| full | high-risk | required, full artifact suite | spec + quality (forked verifier) | all six | + human signoff |

Rules:

- Absent or malformed `risk-tier.json` defaults to the standard lane; lanes
  only loosen gates when explicitly declared.
- Anti-gaming: a light-lane change whose cumulative production diff exceeds
  `escalation_threshold` (default 10 files) raises
  `lane-escalation-required:*` and must re-classify before proceeding.
- Lane classification itself is deterministic (path triggers + heuristics in
  `risk-tier.js`); the `reason` field records why.

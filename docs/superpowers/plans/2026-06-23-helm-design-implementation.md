# Helm Design Implementation Plan

> Superseded by `docs/superpowers/plans/2026-06-23-helm-plugin-suite-implementation.md`. This earlier draft treated Helm too much like one plugin with multi-plugin repository support. The accepted architecture is a marketplace repository containing multiple Helm lifecycle plugins.

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the Helm Claude Code plugin described in `docs/design.md`: a no-fallback OpenSpec lifecycle plugin covering requirements, prototype, development, six-domain verification, operations, doctor, archive, and bilingual docs.

**Architecture:** Keep the current CommonJS script architecture and grow it through small contract modules under `scripts/`. `scripts/helm-lib.js` remains the shared filesystem/runtime utility layer; new lifecycle scripts produce machine-readable artifacts, and existing hooks/commands/skills call those contracts instead of inferring state from chat. Plugin installation code must distinguish marketplace repository root from individual plugin root because a Claude Code plugin repository can contain multiple plugins.
...
All doctor, install verification, release packaging, and host compatibility checks resolve the plugin root through marketplace metadata. They must not assume the repository root is the Helm plugin root.
...
- `scripts/workflow-state.js`: builds `openspec/.helm/workflow-state.json` and legal action tables.
...
- `scripts/helm-doctor.js`: verifies plugin install surface, hooks, scripts, OpenSpec, state, context manifests, and JSON health output.
- `scripts/helm-lib.js`: add path helpers, strict OpenSpec detection, JSONL writer, Markdown section helpers, and git state helper.
- `scripts/affordances.js`: replace filesystem fallback logic with workflow-state driven affordances.
- `scripts/helm-guard.js`: enforce no-fallback production write blocking through contracts.
- `scripts/helm-session-start.js`: run lightweight OpenSpec/doctor detection on every session.
...
- `commands/helm-requirements.md`, `commands/helm-prototype.md`, `commands/helm-implement.md`, `commands/helm-release.md`, `commands/helm-doctor.md`: new stage commands.
- Existing `commands/helm.md`, `commands/helm-status.md`, `commands/helm-verify.md`, `commands/helm-archive.md`: update routing and blockers.
- New skills under `skills/`: `using-helm`, `requirements`, `foundation-spec`, `prototype`, `prototype-verify`, `prototype-handoff`, `before-dev`, `scope-lock`, `vertical-slice-tasking`, `verify-plan`, six `verify-*` skills, `ops-readiness`, `release-plan`, `install-verify`, `update-policy`, `compatibility-matrix`, `branch-finish`, `deploy`, `rollback`, `monitor`, `postmortem`, `update-spec`, `doctor`, `debug`, `break-loop`.
- `README.md`: English README.
- `README.zh-CN.md`: Chinese README.
- `docs/design.md`: keep design aligned only when implementation reveals a contract mismatch.
- `docs/helm-plan-review.html`: update only when major plan sections change.
- New fixture scripts under `tests/`: one script per contract slice.
- New fixtures under `tests/fixtures/`: focused valid and invalid OpenSpec/Helm projects.
...
assert.strictEqual(contextPath.endsWith('openspec/.helm/context/verify-context.jsonl'), true);
...
function helmStatePath(root, relativePath) {
  return path.join(root, 'openspec', '.helm', relativePath);
}

function contextManifestPath(root, stage) {
  return helmStatePath(root, path.join('context', `${stage}-context.jsonl`));
}
...
- [ ] **Step 4: Extend `scripts/helm-lib.js` with focused helpers**
...
```bash
git add scripts/contracts.js scripts/helm-lib.js tests/run-contract-fixtures.sh
git commit -m "feat: add helm lifecycle contract helpers"
```
...
node "$ROOT/scripts/foundation-specs.js" --json "$VALID" >/tmp/helm-foundation-valid.json
jq -e '.ok == true' /tmp/helm-foundation-valid.json >/dev/null
jq -e '.specs | length == 4' /tmp/helm-foundation-valid.json >/dev/null

set +e
node "$ROOT/scripts/foundation-specs.js" --json "$MISSING" >/tmp/helm-foundation-missing.json
STATUS=$?
set -e
[[ "$STATUS" == "2" ]]
jq -e '.ok == false' /tmp/helm-foundation-missing.json >/dev/null
jq -e '.blockers[] | select(. == "openspec-missing")' /tmp/helm-foundation-missing.json >/dev/null
...
MD
set +e
node "$ROOT/scripts/foundation-specs.js" --json "$INVALID" >/tmp/helm-foundation-invalid.json
STATUS=$?
set -e
[[ "$STATUS" == "2" ]]
jq -e '.blockers[] | select(. == "missing-foundation-spec:system-architecture")' /tmp/helm-foundation-invalid.json >/dev/null
jq -e '.specs[] | select(.id == "ui-design" and .ok == false)' /tmp/helm-foundation-invalid.json >/dev/null

echo "helm foundation fixtures ok"
```
...
function validateFoundationSpecs(root = lib.projectRoot()) {
  ...
  if (fs.existsSync(lib.openspecDir(root))) {
    lib.writeJson(path.join(lib.helmDir(root), 'foundation-specs.json'), result);
...
function markdown(result) {
  const lines = ['# Helm Foundation Specs', '', `- ok: ${result.ok}`];
...
In `scripts/helm-guard.js`, require the validator:
...
```bash
git add scripts/foundation-specs.js scripts/affordances.js scripts/helm-guard.js tests/run-foundation-fixtures.sh
...
- Modify: `scripts/affordances.js`
- Modify: `commands/helm-status.md`
...
PROJECT_DIR="$TMP" node "$ROOT/scripts/workflow-state.js" --write --json >/tmp/helm-workflow-state.json
jq -e '.state == "development"' /tmp/helm-workflow-state.json >/dev/null
jq -e '.legal_actions[] | select(. == "verify")' /tmp/helm-workflow-state.json >/dev/null
jq -e '.blocked_actions.implement | length == 0' /tmp/helm-workflow-state.json >/dev/null
jq -e '.required_artifacts[] | select(. == "openspec/changes/add-dark-mode/tasks.md")' /tmp/helm-workflow-state.json >/dev/null
jq -e '.state == "development"' "$TMP/openspec/.helm/workflow-state.json" >/dev/null

echo "helm workflow state fixtures ok"
```
...
In `commands/helm-status.md`, make the first command:
...
```text
helm workflow state fixtures ok
helm smoke ok
```
...
```bash
git add scripts/workflow-state.js scripts/affordances.js commands/helm-status.md skills/status/SKILL.md tests/run-workflow-state-fixtures.sh
git commit -m "feat: add helm workflow state machine"
```
...
**Files:**
- Modify: `scripts/helm-session-start.js`
- Modify: `scripts/helm-guard.js`
- Modify: `scripts/affordances.js`
...
set +e
PROJECT_DIR="$TMP" node "$ROOT/scripts/helm-guard.js" <"$TMP/write.json" >/tmp/helm-no-fallback-write.out 2>/tmp/helm-no-fallback-write.err
STATUS=$?
set -e
[[ "$STATUS" == "2" ]]
grep -q "openspec" /tmp/helm-no-fallback-write.err

node "$ROOT/scripts/affordances.js" --json "$TMP" >/tmp/helm-no-fallback-affordances.json
jq -e '.workflow_state == "no_openspec"' /tmp/helm-no-fallback-affordances.json >/dev/null
jq -e '.actions[] | select(.id == "bootstrap" and .state == "ready")' /tmp/helm-no-fallback-affordances.json >/dev/null
jq -e '.actions[] | select(.id == "implement" and .state == "blocked")' /tmp/helm-no-fallback-affordances.json >/dev/null

HELM_DISABLE_OPENSPEC=1 node "$ROOT/scripts/affordances.js" --json "$TMP" >/tmp/helm-no-fallback-disabled.json
jq -e '.workflow_state == "no_openspec"' /tmp/helm-no-fallback-disabled.json >/dev/null

PROJECT_DIR="$TMP" node "$ROOT/scripts/helm-session-start.js" >/tmp/helm-session-start.out 2>/tmp/helm-session-start.err
grep -q "Helm detected missing openspec" /tmp/helm-session-start.err

echo "helm no fallback fixtures ok"
```
...
Replace `scripts/helm-session-start.js` with:
...
function main() {
  const root = lib.projectRoot();
  if (!fs.existsSync(lib.openspecDir(root))) {
    console.error(`Helm detected missing openspec/ in ${root}. Production edits are blocked until OpenSpec is initialized.`);
    process.exit(0);
  }
  lib.event(root, 'session.start', { cwd: root });
  const workflow = require('./workflow-state').buildWorkflowState(root);
  lib.writeJson(require('path').join(lib.helmDir(root), 'workflow-state.json'), workflow);
}
...
In `scripts/helm-guard.js`, after `const root = lib.projectRoot();`, add:

```js
  if (!fs.existsSync(lib.openspecDir(root))) {
    lib.event(root, 'hook.deny', { reason: 'openspec-missing' });
    deny('production edits require openspec/. Run Helm initialization before editing code.');
  }
```
...
```bash
HELM_DISABLE_OPENSPEC=1 node "$ROOT/scripts/affordances.js" --json "$PROJECT" >/tmp/helm-disabled-affordances.json
jq -e '.openspec_status.ok == false' /tmp/helm-disabled-affordances.json >/dev/null
jq -e '.openspec_status.error == "disabled"' /tmp/helm-disabled-affordances.json >/dev/null
```
...
```text
helm no fallback fixtures ok
helm hook fixtures ok
helm openspec fixtures ok
```

- [ ] **Step 7: Commit**

```bash
git add scripts/helm-session-start.js scripts/helm-guard.js scripts/affordances.js tests/run-no-fallback-fixtures.sh tests/run-hook-fixtures.sh tests/run-openspec-fixtures.sh
...
**Files:**
- Create: `commands/helm-requirements.md`
...
- Modify: `commands/helm.md`
- Modify: `skills/helm-router/SKILL.md`
...
set +e
PROJECT_DIR="$TMP" node "$ROOT/scripts/requirements-contract.js" --json >/tmp/helm-requirements-missing.json
STATUS=$?
set -e
[[ "$STATUS" == "2" ]]
jq -e '.blockers[] | select(. == "requirements.md")' /tmp/helm-requirements-missing.json >/dev/null
...
{
  "schema": "helm.requirements.specMap.v1",
...
{
  "schema": "helm.requirements.componentImpact.v1",
  "new_components": [],
...
PROJECT_DIR="$TMP" node "$ROOT/scripts/requirements-contract.js" --json >/tmp/helm-requirements-valid.json
jq -e '.ok == true' /tmp/helm-requirements-valid.json >/dev/null

echo "helm requirements fixtures ok"
```
...
function validateRequirements(root = lib.projectRoot()) {
  ...
  if (specMap && specMap.schema !== 'helm.requirements.specMap.v1') blockers.push('spec-map-schema');
  ...
  if (componentMap && componentMap.schema !== 'helm.requirements.componentImpact.v1') blockers.push('component-impact-schema');
...
```markdown
---
description: Run the Helm requirements stage after foundation specs are valid
argument-hint: "[change intent]"
---

You are running the Helm requirements stage.
...
In `skills/helm-router/SKILL.md`, change routing:
...
In `commands/helm.md`, add:

```markdown
If the user asks to define requirements, first run `/helm-requirements`.
...
```text
helm requirements fixtures ok
```

- [ ] **Step 7: Commit**

```bash
git add commands/helm-requirements.md skills/requirements/SKILL.md skills/foundation-spec/SKILL.md scripts/requirements-contract.js tests/run-requirements-fixtures.sh commands/helm.md skills/helm-router/SKILL.md
...
**Files:**
- Create: `commands/helm-prototype.md`
...
set +e
PROJECT_DIR="$TMP" node "$ROOT/scripts/prototype-contract.js" --json >/tmp/helm-prototype-missing.json
STATUS=$?
set -e
[[ "$STATUS" == "2" ]]
jq -e '.blockers[] | select(. == "prototype-manifest.json")' /tmp/helm-prototype-missing.json >/dev/null
...
{
  "schema": "helm.prototype.decision.v1",
  "decision": "approved",
...
PROJECT_DIR="$TMP" node "$ROOT/scripts/prototype-contract.js" --json >/tmp/helm-prototype-valid.json
jq -e '.ok == true' /tmp/helm-prototype-valid.json >/dev/null

echo "helm prototype fixtures ok"
```
...
function validatePrototype(root = lib.projectRoot()) {
  ...
  if (manifest) {
    if (manifest.schema !== 'helm.prototype.manifest.v1') blockers.push('prototype-manifest-schema');
...
```markdown
---
description: Build and verify isolated Helm prototype artifacts
...
Write `prototype/verifier-report.json` with schema `helm.prototype.verifier.v1` and status `green`, `red`, or `blocked`.
...
```text
helm prototype fixtures ok
```

- [ ] **Step 7: Commit**

```bash
git add commands/helm-prototype.md skills/prototype/SKILL.md skills/prototype-verify/SKILL.md skills/prototype-handoff/SKILL.md scripts/prototype-contract.js tests/run-prototype-fixtures.sh scripts/workflow-state.js
...
**Files:**
- Create: `commands/helm-implement.md`
...
set +e
PROJECT_DIR="$TMP" node "$ROOT/scripts/development-contract.js" --json >/tmp/helm-development-missing.json
STATUS=$?
set -e
[[ "$STATUS" == "2" ]]
jq -e '.blockers[] | select(. == "development/handoff-to-verify.md")' /tmp/helm-development-missing.json >/dev/null

mkdir -p "$CHANGE_DIR/development/tasks/001-theme-toggle"
cat >"$CHANGE_DIR/development/prototype-promotion-map.json" <<'JSON'
{
  "schema": "helm.development.prototypePromotion.v1",
...
PROJECT_DIR="$TMP" node "$ROOT/scripts/development-contract.js" --json >/tmp/helm-development-valid.json
jq -e '.ok == true' /tmp/helm-development-valid.json >/dev/null

echo "helm development fixtures ok"
```
...
function validateDevelopment(root = lib.projectRoot()) {
  ...
  if (promotion && promotion.schema !== 'helm.development.prototypePromotion.v1') blockers.push('prototype-promotion-map-schema');
...
```markdown
---
description: Implement a Helm change after requirements and prototype gates pass
...
---
name: before-dev
description: Prepare Helm development artifacts before production edits
allowed-tools:
  - Read
  - Write
  - Edit
...
---
name: scope-lock
description: Create or repair scope.json for Helm development
allowed-tools:
  - Read
  - Write
  - Edit
...
description: Split Helm implementation into vertical slices
allowed-tools:
  - Read
  - Write
  - Edit
...
In `scripts/helm-guard.js`, keep existing `scope.json` enforcement and add this before production writes:
...
```bash
git add commands/helm-implement.md skills/before-dev/SKILL.md skills/scope-lock/SKILL.md skills/vertical-slice-tasking/SKILL.md scripts/development-contract.js scripts/helm-guard.js scripts/workflow-state.js tests/run-development-fixtures.sh
...
- Modify: `commands/helm-verify.md`
...
set +e
PROJECT_DIR="$TMP" node "$ROOT/scripts/verify-domains.js" validate --json >/tmp/helm-verify-missing.json
STATUS=$?
set -e
[[ "$STATUS" == "2" ]]
jq -e '.blockers[] | select(. == "verify/plan.json")' /tmp/helm-verify-missing.json >/dev/null

cat >"$VERIFY_DIR/plan.json" <<'JSON'
{"schema":"helm.verify.plan.v1","required_domains":["facticity","static","unit","redteam","e2e","sensory"]}
JSON
...
{"schema":"helm.verify.traceability.v1","mapped_changes":[{"file":"src/ui/theme.ts","requirement":"theme-toggle","domains":["unit","e2e"]}],"unmapped_changes":[]}
JSON
cat >"$VERIFY_DIR/blocker-classification.jsonl" <<'JSONL'
JSONL
cat >"$VERIFY_DIR/receipt.json" <<'JSON'
{"schema":"helm.verify.receipt.v1","covered_scope":["src/ui/theme.ts"],"uncovered_scope":[],"residual_risk":[],"confidence":"A"}
JSON
cat >"$VERIFY_DIR/behavior-evals/report.json" <<'JSON'
{"schema":"helm.verify.behaviorEvals.v1","status":"green","scenarios":[]}
JSON

for domain in facticity static unit redteam e2e sensory; do
  cat >"$VERIFY_DIR/$domain/report.json" <<JSON
{"schema":"helm.verify.domainReport.v1","domain":"$domain","verdict":"green","required_fixes":[],"residual_risk":[]}
JSON
done

cat >"$VERIFY_DIR/unit/test-quality-rubric.json" <<'JSON'
{"schema":"helm.verify.testQuality.v1","blocking_findings":[]}
JSON
...
PROJECT_DIR="$TMP" node "$ROOT/scripts/verify-domains.js" validate --json >/tmp/helm-verify-valid.json
jq -e '.ok == true' /tmp/helm-verify-valid.json >/dev/null

echo "helm verify domain fixtures ok"
```
...
function writeAggregate(root = lib.projectRoot()) {
  ...
  const report = {
    schema: 'helm.verify.aggregate.v1',
    generated_at: new Date().toISOString(),
    active_change: change,
  ...
  fs.writeFileSync(path.join(verifyDir, 'aggregate-report.md'), `# Helm Aggregate Verify Report\n\n- verdict: ${report.verdict}\n- blockers: ${report.blockers.join(', ') || '-'}\n`);
  return report;
}
...
Write `verify/static/report.json` with schema `helm.verify.domainReport.v1`.
...
In `commands/helm-verify.md`, add:
...
```text
helm verify domain fixtures ok
```

- [ ] **Step 8: Commit**

```bash
git add scripts/verify-domains.js scripts/verify.js commands/helm-verify.md skills/verify-plan/SKILL.md skills/verify-facticity/SKILL.md skills/verify-static/SKILL.md skills/verify-unit/SKILL.md skills/verify-redteam/SKILL.md skills/verify-e2e/SKILL.md skills/verify-sensory/SKILL.md tests/run-verify-domain-fixtures.sh
...
- Create: `commands/helm-release.md`
...
{"schema":"helm.verify.aggregate.v1","verdict":"green","blockers":[]}
JSON
...
set +e
PROJECT_DIR="$TMP" node "$ROOT/scripts/operations-gate.js" --json >/tmp/helm-ops-missing.json
STATUS=$?
set -e
[[ "$STATUS" == "2" ]]
jq -e '.blockers[] | select(. == "operations/readiness.json")' /tmp/helm-ops-missing.json >/dev/null
...
{"schema":"helm.ops.archiveGate.v1","verdict":"green","blockers":[]}
JSON
touch "$OPS/archive-log.jsonl"

PROJECT_DIR="$TMP" node "$ROOT/scripts/operations-gate.js" --json >/tmp/helm-ops-valid.json
jq -e '.ok == true' /tmp/helm-ops-valid.json >/dev/null

PROJECT_DIR="$TMP" node "$ROOT/scripts/archive-gate.js" >/tmp/helm-archive-ops.out

echo "helm operations fixtures ok"
```
...
```markdown
---
description: Run Helm operations release gate
...
---
name: ops-readiness
description: Write Helm operations readiness artifact
allowed-tools:
  - Read
  - Write
  - Bash
...
Write `operations/readiness.json` with schema `helm.ops.readiness.v1`.
...
- Create: `scripts/helm-doctor.js`
- Create: `commands/helm-doctor.md`
...
node "$ROOT/scripts/helm-doctor.js" --plugin-root "$ROOT" --json >/tmp/helm-doctor.json
jq -e '.ok == true' /tmp/helm-doctor.json >/dev/null
jq -e '.plugin.root == "'"$ROOT"'"' /tmp/helm-doctor.json >/dev/null
jq -e '.marketplace.root == "'"$ROOT"'"' /tmp/helm-doctor.json >/dev/null
jq -e '.marketplace.plugin_source == "./"' /tmp/helm-doctor.json >/dev/null
jq -e '.checks[] | select(.name == "hooks" and .status == "pass")' /tmp/helm-doctor.json >/dev/null
jq -e '.workspaceSupport == "available"' /tmp/helm-doctor.json >/dev/null
jq -e '.configStatus == "configured"' /tmp/helm-doctor.json >/dev/null

MULTI="$TMP/multi-plugin-repo"
mkdir -p "$MULTI/.claude-plugin" "$MULTI/plugins/helm"
cp -R "$ROOT/.claude-plugin" "$MULTI/plugins/helm/.claude-plugin"
cp -R "$ROOT/commands" "$MULTI/plugins/helm/commands"
cp -R "$ROOT/skills" "$MULTI/plugins/helm/skills"
cp -R "$ROOT/hooks" "$MULTI/plugins/helm/hooks"
cp -R "$ROOT/scripts" "$MULTI/plugins/helm/scripts"
...
node "$ROOT/scripts/helm-doctor.js" --marketplace-root "$MULTI" --plugin-name helm --json >/tmp/helm-doctor-multi.json
jq -e '.ok == true' /tmp/helm-doctor-multi.json >/dev/null
jq -e '.marketplace.root == "'"$MULTI"'"' /tmp/helm-doctor-multi.json >/dev/null
jq -e '.marketplace.plugin_source == "plugins/helm"' /tmp/helm-doctor-multi.json >/dev/null
jq -e '.plugin.root == "'"$MULTI/plugins/helm"'"' /tmp/helm-doctor-multi.json >/dev/null

set +e
node "$ROOT/scripts/helm-doctor.js" --marketplace-root "$MULTI" --plugin-name missing --json >/tmp/helm-doctor-missing.json
STATUS=$?
set -e
[[ "$STATUS" == "2" ]]
jq -e '.ok == false' /tmp/helm-doctor-missing.json >/dev/null
jq -e '.checks[] | select(.name == "plugin-resolution" and .status == "fail")' /tmp/helm-doctor-missing.json >/dev/null

echo "helm doctor fixtures ok"
```
...
Expected: FAIL with `Cannot find module` for `scripts/helm-doctor.js`.
...
function resolvePluginRepository(options = {}) {
  const requestedPluginName = options.pluginName || 'helm';
...
function main() {
  ...
  const result = resolvePluginRepository({
    ...
    pluginName: argValue(args, '--plugin-name', 'helm')
  });
  process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
  process.exit(result.ok ? 0 : 2);
}
...
- [ ] **Step 4: Create `scripts/helm-doctor.js`**
...
function doctor(options = {}) {
  ...
  checks.push(check('scripts', fs.existsSync(path.join(pluginRoot, 'scripts/affordances.js')) && fs.existsSync(path.join(pluginRoot, 'scripts/helm-guard.js'))));
  const ok = checks.every((item) => item.status === 'pass');
  return {
    schema: 'helm.doctor.v1',
    generated_at: new Date().toISOString(),
    ok,
...
```markdown
---
description: Verify Helm plugin installation and project health
argument-hint: "[--json]"
---
...
```bash
node "$CLAUDE_PLUGIN_ROOT/scripts/helm-doctor.js" --plugin-root "$CLAUDE_PLUGIN_ROOT" --plugin-name helm --json
```

For a marketplace repository containing multiple plugins, run from the marketplace root and select the plugin:

```bash
node plugins/helm/scripts/helm-doctor.js --marketplace-root "$PWD" --plugin-name helm --json
```
...
---
name: doctor
description: Verify Helm install and runtime health
allowed-tools:
  - Bash
  - Read
---
...
If the plugin is installed from a marketplace repository that contains multiple plugins, pass `--marketplace-root <repo>` and `--plugin-name helm`. Doctor must verify the resolved individual plugin root, not every plugin in the marketplace.
...
```text
helm doctor fixtures ok
```

- [ ] **Step 7: Commit**

```bash
git add scripts/plugin-repository.js scripts/helm-doctor.js commands/helm-doctor.md skills/doctor/SKILL.md tests/run-doctor-fixtures.sh
git commit -m "feat: add helm doctor"
```
...
**Files:**
- Create: `skills/using-helm/SKILL.md`
...
for skill in using-helm debug break-loop doctor requirements prototype verify-static ops-readiness; do
...
echo "helm skill surface fixtures ok"
```
...
Expected: FAIL because `using-helm`, `debug`, or `break-loop` skill files do not exist.

- [ ] **Step 3: Create `skills/using-helm/SKILL.md`**

```markdown
---
name: using-helm
description: Bootstrap Helm state and route user intent through OpenSpec lifecycle gates
allowed-tools:
  - Bash
  - Read
---
...
At session start or when Helm is mentioned, run:
...
---
name: debug
description: Investigate repeated Helm workflow failures with evidence
allowed-tools:
  - Bash
  - Read
  - Write
...
Write a debug note under `openspec/.helm/journal/` containing:
...
---
name: break-loop
description: Stop repeated patch loops and force a Helm root-cause reset
allowed-tools:
  - Bash
  - Read
  - Write
...
Write a journal entry under `openspec/.helm/journal/` with the exact failing command, last three attempts, suspected wrong assumption, and next smallest experiment.
```
...
```text
helm skill surface fixtures ok
```

- [ ] **Step 6: Commit**

```bash
git add skills/using-helm/SKILL.md skills/debug/SKILL.md skills/break-loop/SKILL.md tests/run-skill-surface-fixtures.sh hooks/hooks.json .claude-plugin/plugin.json
git commit -m "feat: add helm bootstrap and debug skills"
```
...
Helm is a Claude Code plugin for OpenSpec-driven engineering work. It routes work through requirements, prototype, development, six-domain verification, operations, and archive gates.
...
Claude Code can install multiple plugins from one marketplace repository. Helm distinguishes:
...
Helm updates the current host by default. Updating every registered host requires an explicit all-host request.
```
...
Helm 是构建在 OpenSpec 之上的 Claude Code 工程生命周期插件。

## 主流程

Helm 将工作路由到需求、原型、开发、六域验证、运维和归档阶段。
...
Claude Code 支持一个 marketplace 仓库下包含多个插件。Helm 必须区分：
...
Helm 默认只更新当前宿主。更新所有已登记宿主必须由用户显式要求。
```
...
- Added strict no-fallback gates for required OpenSpec and Helm artifacts.
- Added marketplace repository support so install verification resolves the Helm plugin root inside multi-plugin repositories.
...
git commit -m "docs: add bilingual helm lifecycle documentation"
```
...
echo "helm full fixture suite ok"
```
...
- No generated `.helm/events.jsonl` fixture noise is staged.

- [ ] **Step 4: Update design docs only for implementation deltas**

If implementation changed a contract name, update `docs/design.md` and `docs/helm-plan-review.html` to match the code. Use exact code names from scripts:

```bash
rg -n "plugin-repository|requirements-contract|prototype-contract|development-contract|verify-domains|operations-gate|helm-doctor" docs scripts
```
...
```bash
git add tests/run-all.sh docs/design.md docs/helm-plan-review.html
git commit -m "test: add full helm lifecycle fixture suite"
```
...
- `helm.requirements.specMap.v1`
- `helm.requirements.componentImpact.v1`
- `helm.prototype.manifest.v1`
- `helm.prototype.verifier.v1`
- `helm.prototype.decision.v1`
- `helm.development.prototypePromotion.v1`
- `helm.verify.plan.v1`
- `helm.verify.domainReport.v1`
- `helm.verify.aggregate.v1`
- `helm.ops.readiness.v1`
- `helm.doctor.v1`

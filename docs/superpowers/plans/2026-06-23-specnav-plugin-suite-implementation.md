...
- Modify: `plugins/specnav-core/specnav-stage.json`
- Test: `tests/run-plugin-suite-resolver-fixtures.sh`

Implementation note: `plugins/specnav-core/specnav-stage.json` promotes the suite resolver as the core plugin's `contracts.suite` entry, so other plugins can discover the marketplace contract through `specnav-core` instead of hard-coding an implementation detail.
...
- Modify: `plugins/specnav-requirements/specnav-stage.json`
- Test: `tests/run-requirements-plugin-fixtures.sh`

Implementation note: `plugins/specnav-requirements/specnav-stage.json` promotes the requirements plugin's implemented `contracts.foundation` and `contracts.requirements` entries from `planned_contracts` once the scripts exist. Requirements plugin scripts import shared helpers from `../../specnav-core/scripts/specnav-lib`; there is no core `contracts.js` module in the current suite.

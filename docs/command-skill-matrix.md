...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Route | `/specnav` | `specnav-core` | `specnav-route`, `specnav-workflow` | `plugin-suite.js`, `affordances.js`, `change-triage.js` | plugin suite, OpenSpec state, user intent | none | reports next legal action and lane | missing plugin, missing OpenSpec, blocked action | target command selected | stage command |
...
| Light change | `/specnav-implement` | `specnav-development` | `specnav-light-change` | `change-triage.js`, `create-light-change.js`, `development-contract.js --mode entry` | user intent, explicit paths, active change | light requirements, acceptance, spec map, component impact map, `prototype/decision.json`, `scope.json`, checkbox `tasks.md` | creates minimal legal artifacts for simple work | missing active change, missing paths, triage not light, invalid checkbox tasks | light entry contract green | edit scoped files, then `/specnav-verify` |
...
| Verification | `/specnav-verify` | `specnav-verification` | six `specnav-verify-*` skills | `verify-domains.js` | development handoff, specs, tests | six-domain evidence, aggregate report, stakeholder HTML report | test/audit execution with reviewable output | missing evidence, stale report, red domain | aggregate green and `verify-report.html` written | `/specnav-release` |

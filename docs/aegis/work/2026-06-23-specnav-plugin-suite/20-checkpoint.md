# SpecNav plugin suite implementation - Checkpoint

- Task ID: 2026-06-23-specnav-plugin-suite
...
- Evidence refs:
- Code quality agent 019ef3dc-c2aa-7cf3-a028-9d498f0baa71 reported duplicate specnav-core commands and missing declared command/skill files.
- Blocked on: none
...
- Current todo: Patch Task 1 core contract surface: workflow-state.js and specnav-doctor.js explicit blockers.
...
- Evidence refs:
- Spec re-review agent 019ef3ed-1930-7e83-ae74-d850cfd2424a reported missing workflow-state.js and specnav-doctor.js.
- Blocked on: none
...
- Completed todos:
- Task 1 spec passed; quality review found /specnav and specnav-router still route to legacy core lifecycle skills.
- Evidence refs:
...
- Current todo: Task 2: Suite Resolver In SpecNav Core.
- Active slice: Task 2: Suite Resolver In SpecNav Core
- Completed todos:
...
- Compatibility status: Current fixtures pass after scripts moved under plugins/specnav-core; plugin-suite remains planned for Task 2.
...
- New risk signals:
- Future Task 3 plan text assumes specnav-doctor and workflow-state are full implementations, but Task 1 added explicit blocking placeholders; Task 3 should replace placeholders deliberately.
...
- Completed todos:
- Task 2 spec review functionally passed but flagged specnav-stage.json plan scope; quality review found flag-value, invalid entry, unreadable file issues.
- Evidence refs:
...
- Scope status: Task 4 stayed inside specnav-requirements, suite layout metadata, and focused fixtures.
...
- New risk signals:
- Task 5 plan text still references a non-existent `specnav-core/scripts/contracts.js`, while the accepted suite architecture and Task 4 implementation use `specnav-lib` plus plugin-local contracts. Task 5 must not import the missing module.
- Advisory decision: continue
...
- Scope status: Task 5 stayed inside specnav-prototype and focused prototype fixtures.
- Compatibility status: Current prototype, requirements, core runtime, resolver, and layout fixtures pass at HEAD 77d4400.
- Retirement status: Prototype placeholder contract and skills are now implemented; `specnav-stage.json` exposes `contracts.prototype`, not `planned_contracts`.
- New risk signals:

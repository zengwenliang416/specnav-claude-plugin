...
jq '.freshness.status = "stale" | .freshness.reasons = ["code_sha:mismatch"]' \
  "$VERIFY_FAIL/openspec/changes/add-dashboard/verify/v2/gate-input.json" \
  >"$TMP_DIR/verify-fail.tmp"
mv "$TMP_DIR/verify-fail.tmp" \
  "$VERIFY_FAIL/openspec/changes/add-dashboard/verify/v2/gate-input.json"
run_gate operations-gate.js "$VERIFY_FAIL" "$TMP_DIR/verify-fail.json" 2
assert_blocker "$TMP_DIR/verify-fail.json" 'verification-release:kernel-gate-not-pass:release'

UNRESOLVED="$TMP_DIR/unresolved"
cp -R "$PROJECT" "$UNRESOLVED"
jq '.open_failure_ids = ["failure-unresolved"]' \
  "$UNRESOLVED/openspec/changes/add-dashboard/verify/v2/gate-input.json" \
  >"$TMP_DIR/unresolved.tmp"
mv "$TMP_DIR/unresolved.tmp" \
  "$UNRESOLVED/openspec/changes/add-dashboard/verify/v2/gate-input.json"

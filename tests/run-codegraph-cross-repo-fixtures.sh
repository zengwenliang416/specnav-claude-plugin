...
node --check "$CODEGRAPH/scripts/cross-repo-guard.js"
node --check "$CODEGRAPH/scripts/cross-repo-announce.js"
...
run_guard() {
  ...
  set +e
  printf '%s' "$payload" | PROJECT_DIR="$MAIN" node "$CODEGRAPH/scripts/cross-repo-guard.js" >"$out" 2>/dev/null
  GUARD_STATUS=$?
  GUARD_OUT="$(cat "$out")"
  set -e
}
...
printf '%s' "{\"tool_name\":\"Grep\",\"tool_input\":{\"pattern\":\"x\",\"path\":\"$INDEXED/src\"}}" \
  | SPECNAV_CROSS_REPO_REDIRECT=0 PROJECT_DIR="$MAIN" node "$CODEGRAPH/scripts/cross-repo-guard.js" >/dev/null 2>&1
STATUS=$?
set -e
[[ "$STATUS" == "0" ]] || { echo "cross-repo: kill switch not honored"; exit 1; }

# Announce: sibling with index -> one compact JSON line naming it.
OUT="$(PROJECT_DIR="$MAIN" node "$CODEGRAPH/scripts/cross-repo-announce.js")"
...
mkdir -p "$LONELY"
OUT="$(PROJECT_DIR="$LONELY" node "$CODEGRAPH/scripts/cross-repo-announce.js")"

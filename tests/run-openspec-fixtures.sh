...
if command -v openspec >/dev/null 2>&1; then
  node "$ROOT/scripts/affordances.js" --json "$PROJECT" >/tmp/helm-openspec-affordances.json
...
HELM_DISABLE_OPENSPEC=1 node "$ROOT/scripts/affordances.js" --json "$PROJECT" >/tmp/helm-fallback-affordances.json
